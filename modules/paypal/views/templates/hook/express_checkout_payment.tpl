	...
	<div class="col-xs-12 col-md-12">
        <p class="payment_module paypal">
        	{if $use_paypal_in_context}
				<a href="javascript:void(0)" onclick="" id="paypal_process_payment" title="{l s='Pay with PayPal' mod='paypal'}" class="bankwire">
			{else}
				<a href="javascript:void(0)" onclick="$('#paypal_payment_form_payment').submit();" title="{l s='Pay with PayPal' mod='paypal'}" class="bankwire">
			{/if}
				...
				{else}
					{if isset($logos.LocalPayPalHorizontalSolutionPP) && $PayPal_payment_method == $PayPal_integral}
						<img src="{$base_dir_ssl|escape:'htmlall':'UTF-8'}themes/furhome16/modules/paypal/views/img/payment-custom-paypal.gif" alt="{if $lang_iso=='el'}Πληρώστε με την κάρτα σας ή μέσω PayPal{else}Pay with your card or your PayPal account{/if}" height="80px" />
					{else}
						<img src="{$base_dir_ssl|escape:'htmlall':'UTF-8'}themes/furhome16/modules/paypal/views/img/payment-custom-paypal.gif" alt="{l s='Pay with your card or your PayPal account' mod='paypal'}" />
					{/if}
            {if isset($braintreeToken)}
            {l s='Pay with PayPal' mod='paypal'}
            {else}
						{if $lang_iso=='el'}Πληρώστε με την κάρτα σας ή μέσω PayPal{else}Pay with your card or your PayPal account{/if}
            {/if}
				{/if}
			</a>
		</p>
